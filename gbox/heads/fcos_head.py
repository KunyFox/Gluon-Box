...
class FCOSHead(AnchorFreeHead):
    ...
    def hybrid_forward_base(self, x, scale, stride, F):

        cls_score, bbox_pred, cls_feat, reg_feat = super().base_hybrid_forward(x, return_feat=True)
    ...
    def loss(self,
        ...
        gt_bboxes = [info.get('bboxes') for info in img_infos]
        gt_labels = [info.get('labels') for info in img_infos]
    ...
    def get_points_base(self,
        ...
        h, w = featmap_size
        x_range = mx.nd.arange(0, w, dtype=dtype, ctx=context).reshape(1, -1)
        y_range = mx.nd.arange(0, h, dtype=dtype, ctx=context).reshape(-1, 1)
        y, x = functions.meshgrid(y_range, x_range)

        points = mx.nd.stack((x.reshape(-1) * stride, y.reshape(-1) * stride),
                             axis=-1) + stride // 2
        return points
    ...
    def get_targets(self, points, gt_bboxes_list, gt_labels_list):
        ...
        points: list of NDArray 
            Points of each fpn level, each has shape (num_points, 2).
        gt_bboxes_list: list of NDArray 
            Ground truth bboxes of each image, each has shape (num_gt, 4).
        gt_labels_list: list of NDArray 
            Ground truth labels of each box, each has shape (num_gt,).

        Returns:
        --------
        concat_lvl_labels: list of NDArray
            Labels of each level. 
        concat_lvl_bbox_targets: list of NDArray
        ...
        expanded_regress_ranges = [
            mx.nd.array(self.regress_ranges[i], ctx=points[i].context).expand_dims(0).repeat(
                repeats=points[i].shape[0], axis=0) for i in range(num_levels)]
    ...
    def get_target_base(self, gt_bboxes, gt_labels, points, regress_ranges,
                           num_points_per_lvl):
        ...
        if num_gts == 0:
            return mx.nd.full((num_points,), self.num_classes, ctx=context), \
                   mx.nd.zeros((num_points, 4), ctx=context)

        areas = (gt_bboxes[:, 2] - gt_bboxes[:, 0]) * (
            gt_bboxes[:, 3] - gt_bboxes[:, 1])
        # TODO: figure out why these two are different
        # areas = areas[None].expand(num_points, num_gts)
        areas = areas.expand_dims(0).repeat(repeats=num_points, axis=0)
        regress_ranges = regress_ranges.expand_dims(1).expand(repeats=num_gts, axis=1)
        gt_bboxes = gt_bboxes.expand_dims(0).repeat(repeats=num_points, axis=0) 
        xs, ys = points[:, 0].expand_dims(1), points[:, 1].expand_dims(1)
        xs = xs.repeat(repeats=num_gts, axis=1)
        ys = ys.repeat(repeats=num_gts, axis=1)

        left = xs - gt_bboxes[..., 0]
        right = gt_bboxes[..., 2] - xs
        top = ys - gt_bboxes[..., 1]
        bottom = gt_bboxes[..., 3] - ys
        bbox_targets = mx.nd.stack((left, top, right, bottom), -1)
        ...
        if self.center_sampling:
            ...
            center_gts = mx.nd.zeros_like(gt_bboxes)
            stride = mx.nd.zeros_like(center_xs)
            ...
            center_gts[..., 0] = mx.nd.where(x_mins > gt_bboxes[..., 0],
                                             x_mins, gt_bboxes[..., 0])
            center_gts[..., 1] = mx.nd.where(y_mins > gt_bboxes[..., 1],
                                             y_mins, gt_bboxes[..., 1])
            center_gts[..., 2] = mx.nd.where(x_maxs > gt_bboxes[..., 2],
                                             gt_bboxes[..., 2], x_maxs)
            center_gts[..., 3] = mx.nd.where(y_maxs > gt_bboxes[..., 3],
                                             gt_bboxes[..., 3], y_maxs)
            ...
            center_bbox = mx.nd.stack(
                (cb_dist_left, cb_dist_top, cb_dist_right, cb_dist_bottom), axis=-1)
        ...
        min_area_inds = areas.argmin(axis=1)
    ...
    def centerness_target(self, pos_bbox_targets):
        ...
        centerness_targets = (
            left_right.min(axis=-1)[0] / left_right.max(axis=-1)[0]) * (
                top_bottom.min(axis=-1)[0] / top_bottom.max(axis=-1)[0])
        return mx.nd.sqrt(centerness_targets)
